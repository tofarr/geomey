...
export const pointSpec = () => {
  ...
  it("generates wkt", () => {
    const point = Point.valueOf(1, 2);
    const format = new Intl.NumberFormat('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format;
  ...
  it("relates to points", () => {
    ...
    expect(point.relatePoint(0.8, 2, TOLERANCE)).to.equal(A_OUTSIDE_B | B_OUTSIDE_A)
    expect(point.relatePoint(1, 1.8, TOLERANCE)).to.equal(A_OUTSIDE_B | B_OUTSIDE_A)
    expect(point.relatePoint(1.2, 2, TOLERANCE)).to.equal(A_OUTSIDE_B | B_OUTSIDE_A)
    expect(point.relatePoint(1, 2.2, TOLERANCE)).to.equal(A_OUTSIDE_B | B_OUTSIDE_A)
  ...
  it("relates to geometries", () => {
    ...
    expect(point.relate(rectangle, TOLERANCE)).to.equal(A_INSIDE_B | B_OUTSIDE_A)
  ...
  it("unions successfully", () => {
    ...
    expect(point.union(Point.valueOf(2.01, 3.01), TOLERANCE).toWkt()).to.equal("POINT(2 3)")
    expect(point.union(Point.valueOf(1, 2), TOLERANCE).toWkt()).to.equal("MULTIPOINT(1 2, 2 3)")
    const rectangle = new Rectangle(1, 2, 3, 4)
    expect(point.union(rectangle, TOLERANCE)).to.equal(rectangle)
    const result = point.union(new Rectangle(6, 7, 8, 9), TOLERANCE)
    expect(result.toWkt()).to.equal("GEOMETRYCOLLECTION(POLYGON((6 7, 8 7, 8 9, 6 9, 6 7)),POINT(2 3))")
  });
  it("intersection successfully", () => {
    const point = Point.valueOf(2, 3)
    expect(point.intersection(point, TOLERANCE)).to.equal(point)
    expect(point.intersection(Point.valueOf(2.01, 3.01), TOLERANCE).toWkt()).to.equal("POINT(2 3)")
  ...
  it("less successfully", () => {
    ...
    expect(point.less(Point.valueOf(2.01, 3.01), TOLERANCE).toWkt()).to.equal("POINT(2 3)")
  ...
  it("xor successfully", () => {
    ...
    expect(point.xor(Point.valueOf(1, 2), TOLERANCE).toWkt()).to.equal("MULTIPOINT(1 2, 2 3)")
    const rectangle = new Rectangle(1, 2, 3, 4)
    expect(point.xor(rectangle, TOLERANCE).toWkt()).to.equal("POLYGON((1 2, 3 2, 3 4, 1 4, 1 2))")
