...
export function less(a: Geometry, b: Geometry, tolerance: Tolerance): Geometry | null {
  ...
  for(let n = 0; n < toRemove.length; ){
    rings.removeLink(toRemove[n++], toRemove[n++], toRemove[n++], toRemove[n++])
  }

  rings.forEachVertex((vertex) => { relink(vertex, rings, touchBoth) })
