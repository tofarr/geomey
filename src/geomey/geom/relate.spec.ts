...
export const relateSpec = () => {
  ...
  it("line segment overlaps another", () => {
    ...
    expect(a.relate(b, TOLERANCE)).to.equal(TOUCH | A_OUTSIDE_B | B_OUTSIDE_A);
  });
  ...
  it("multilinestring intersects multilinestring", () => {
    ...
    expect(a.relate(b, TOLERANCE)).to.equal(A_OUTSIDE_B | B_OUTSIDE_A | TOUCH);
    expect(b.relate(a, TOLERANCE)).to.equal(A_OUTSIDE_B | B_OUTSIDE_A | TOUCH);
    const c = new MultiLineString([new LineString([16, 0, 16, 10, 18, 10])]);
  ...
  it("linearring contains lineString", () => {
    const a = new LinearRing([0, 0, 100, 0, 100, 100, 0, 100]);
    const b = new LineString([20, 20, 80, 20, 80, 80, 20, 80]);
  ...
  it("linearring intersects lineString", () => {
    const a = new LinearRing([20, 20, 80, 20, 80, 80, 20, 80]);
    const b = new LineString([0, 0, 0, 50, 100, 50]);
    expect(a.relate(b, TOLERANCE)).to.equal(
      A_OUTSIDE_B | B_INSIDE_A | B_OUTSIDE_A | TOUCH,
    );
    expect(b.relate(a, TOLERANCE)).to.equal(
      A_INSIDE_B | A_OUTSIDE_B | B_OUTSIDE_A | TOUCH,
    );
  });

  it("linearring contains linearring", () => {
    const a = new LinearRing([0, 0, 100, 0, 100, 100, 0, 100]);
    const b = new LinearRing([20, 20, 80, 20, 80, 80, 20, 80]);
    expect(a.relate(b, TOLERANCE)).to.equal(
      A_INSIDE_B | A_OUTSIDE_B | B_INSIDE_A,
    );
    expect(b.relate(a, TOLERANCE)).to.equal(
      A_INSIDE_B | B_INSIDE_A | B_OUTSIDE_A,
    );
  });

  it("linearring intersects linearring", () => {
    const a = new LinearRing([0, 0, 100, 0, 100, 100, 0, 100]);
    const b = new LinearRing([50, 50, 150, 50, 150, 150, 50, 150]);
  ...
  it("linearring disjoint linearring", () => {
    const a = new LinearRing([0, 0, 100, 0, 100, 100, 0, 100]);
    const b = new LinearRing([200, 0, 300, 0, 300, 100, 200, 100]);
    expect(a.relate(b, TOLERANCE)).to.equal(DISJOINT);
    expect(b.relate(a, TOLERANCE)).to.equal(DISJOINT);
  });
  ...
  it("linearring touch inside linearring", () => {
    const a = new LinearRing([0, 0, 100, 0, 100, 100, 0, 100]);
    const b = new LinearRing([0, 50, 50, 0, 100, 50, 50, 100]);
    expect(a.relate(b, TOLERANCE)).to.equal(
      A_INSIDE_B | A_OUTSIDE_B | B_INSIDE_A | TOUCH,
    );
    expect(b.relate(a, TOLERANCE)).to.equal(
      A_INSIDE_B | B_INSIDE_A | B_OUTSIDE_A | TOUCH,
    );
  });
  ...
  it("polygon contains polygon", () => {
    ...
    const b = new Polygon(new LinearRing([30, 30, 70, 30, 70, 70, 30, 70]), [
