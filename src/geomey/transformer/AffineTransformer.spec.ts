...
export const affineTransformerSpec = () => {
  it ("prevents init with invalid parameters", () => {
    expect(() => { new AffineTransformer(Infinity, 0, 0, 0, 1, 0) }).to.throw(Error)
    expect(() => { new AffineTransformer(1, 0, 0, 0, 1, NaN) }).to.throw(Error)
    expect(() => { new AffineTransformer(1, 0, 0, 0, Infinity, 0) }).to.throw(Error)
  })
  it("identity transform does not modify coordinates", () => {
    ...
    expect(AffineTransformer.IDENTITY.transformAll([1,2,3,4])).to.eql([1,2,3,4])
  });
  ...
  it("explicit scale by 0 throws an error", () => {
    expect(() => { AffineTransformer.IDENTITY.scale(0) }).to.throw(Error)
    expect(() => { AffineTransformer.IDENTITY.scale(2, 0) }).to.throw(Error)
    expect(() => { AffineTransformer.IDENTITY.scale(0, 2) }).to.throw(Error)
  })
  ...
  it("rotate 90 degree blocks rotates", () => {
    ...
    expect(transform.transform(-1, 0)).to.eql([0, -1])
    expect(transform.transform(0, -1)).to.eql([1, 0])
    expect(AffineTransformer.IDENTITY.rotateDegrees(0).transform(1, 0)).to.eql([1, 0])
    expect(AffineTransformer.IDENTITY.rotateDegrees(180).transform(1, 0)).to.eql([-1, 0])
    expect(AffineTransformer.IDENTITY.rotateDegrees(270).transform(1, 0)).to.eql([0, -1])
  });
  ...
  it("add combines matrices in the correct order", () => {
    ...
    const transform3 = AffineTransformer.IDENTITY.translate(-1, -2).scale(2).translate(1, 2)
  ...
  it("get inverse inverts transformation", () => {
    const transform = AffineTransformer.IDENTITY.scaleAround(2, 4, 8).getInverse()
