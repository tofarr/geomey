...
export class LineSegment implements Geometry {
    ...
    relate(other: Geometry, tolerance: Tolerance): Relation {
        ...
        if (other instanceof Point) {
            return relatePointToLineSegment(other.x, other.y, this.ax, this.ay, this.bx, this.by, tolerance)
        }
...
export function intersectionLineSegment(
    ...
    if ((comparePointsForSort(iax, iay, jax, jay) || comparePointsForSort(ibx, iby, jbx, jby)) > 0) {
        [iax, iay, ibx, iby, jax, jay, jbx, jby] = [jax, jay, jbx, jby, iax, iay, ibx, iby,]
    }
...
export function projectPointOntoLineSegment(x: number, y: number, ax: number, ay: number, bx: number, by: number, tolerance: Tolerance): Point | null {
...
export function pointTouchesLineSegment(x: number, y: number, ax: number, ay: number, bx: number, by: number, tolerance: Tolerance) {
...
export function relatePointToLineSegment(x: number, y: number, ax: number, ay: number, bx: number, by: number, tolerance: Tolerance): Relation {
    let result = UNKNOWN
...
export function relateLineSegments(
    ...
    return (
        (intersectionLineSegment(iax, iay, ibx, iby, jax, jay, jbx, jby, tolerance) ? TOUCH : UNKNOWN) |
        ...
        flipAB((
            ...
            relatePointToLineSegment(jbx, jby, iax, iay, ibx, iby, tolerance)
        ) as Relation)
    ) as Relation
}
