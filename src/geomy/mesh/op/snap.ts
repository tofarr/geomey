...
export function getSnapEndIndex(vertices: Vertex[], startIndex: number, snapTolerance: Tolerance): number{
    const result = []
    let { length } = vertices
...
export function snapVertices(vertices: Vertex[], startIndex: number, endIndex: number, mesh: Mesh) {
...
export function createSnapVertex(vertices: Vertex[], startIndex: number, endIndex: number, mesh: Mesh): Vertex {
